...
import type { YouTubeVideoVariant } from '@/shared/media/YouTubeVideoCard'
    ...
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName={className ?? 'gap-sm'}>
      {items.map((item, index) => (
        <View key={item.id} className={cardClassName ?? (variant === 'rail' ? 'w-[68%]' : 'w-[48%]')}>
          <ConnectedVideoCard item={item} variant={variant} onPress={onItemPress ? () => onItemPress(index) : undefined} />
        </View>
      ))}
    </ScrollView>
  )
