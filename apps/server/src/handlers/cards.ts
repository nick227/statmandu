...
export async function listRecentPublicCards(request: any, reply: any) {
  const cards = await cardService.listRecentPublicCards(request.user?.id)
  return reply.send({ data: cards })
}

export async function getCard(request: any, reply: any) {
  const card = await cardService.getCard(request.params.cardId, request.user?.id, request.user?.role === 'ADMIN')
...
export async function listCardsForAthlete(request: any, reply: any) {
  const cards = await cardService.listCardsForAthlete(request.params.athleteProfileId, request.user?.id)
  return reply.send({ data: cards })
}
