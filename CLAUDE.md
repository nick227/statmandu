...
- [x] **Image upload, spectator reactions, and account-capabilities UI** — mobile consumption of the backend module above:
  - `ConnectedImageUploadButton` (`modules/media/`) — wraps `expo-image-picker` + `useUploadImage`, wired into `PlayerProfileScreen`'s editor (`usage="AVATAR"`) and `ConnectedSourcesPanel` (`usage="EVIDENCE"`, so a dispute/source claim can attach a photo, not just a link).
  - Spectator reactions: `useSpectatorGameReaction` (per-mount random `deviceId`, no persistence — matches the backend's ephemeral, unauthenticated model), `SpectatorReactionToolbar` (three tap targets), `FloatingReactionsOverlay` (Reanimated float-and-fade emoji burst driven off `GameSnapshot.recentReactions`, de-duped by id via a `Set` ref so a poll tick doesn't replay reactions already shown) — both wired into `BroadcastDisplayScreen` alongside the existing scoreboard/timeline.
  - `AccountScreen`/`useAccountSession` (already-existing UI, see below) now have a real backing endpoint. `useAthleteOnboarding`'s Publish step attaches the highlight video (`useAttachYouTubeMedia`) after player creation, replacing "media attachment comes next" placeholder copy; guarded by `createdPlayerId`/`mediaAttached` state so retrying `publish()` after a partial failure doesn't double-create the player. `PlayerProfileScreen` also gained a small jersey/height/class "vitals" row.
  - **Caveat resolved, not newly introduced**: `AccountScreen.tsx`/`useAccountSession.ts` were already committed in an earlier session expecting `useMeCapabilities` to exist — it didn't, until the backend commit above. See "Verified Working" for the full broken-`HEAD` finding.
