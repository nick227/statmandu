...
- `pnpm --filter server test` — **113/113 passing**, 15 test files, one per OpenAPI tag
- `pnpm --filter mobile test` — **34/34 passing**, 6 test files (Jest + `jest-expo` + RNTL, harness stood up in an earlier session — see Frontend module list; `predictNext`/`resolvePredictedSelection` suites added during the Live Capture redesign)
